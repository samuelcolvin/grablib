...
def test_cat_regex(tmpworkdir):
    ...
    Grab().build()
    assert gettree(tmpworkdir.join('built_at')) == {'f.min.js': '/* === foo.js === */\nvar v="!new_value js";\n'}
...
def test_sass(tmpworkdir):
    ...
    Grab().build()
    assert gettree(tmpworkdir.join('built_at')) == {
        'css': {'foo.css': 'a{color:red}.foo .bar{color:black;width:10px}\n'}
    }
    ...
    mktree(
        ...
        build:
          sass:
            "css": "sass_dir"
        """,
            'sass_dir': {'foo.scss': 'span {color: white}'},
        },
    )
    Grab().build()
        ...
        build:
          sass:
            css:
              ...
              exclude: 'adir/.*$'
        """,
            'sass_dir': {'adir/bar.scss': '.bar { color: red};', 'foo.scss': '.foo { color: black;}'},
        },
    )
    Grab().build()
    assert gettree(tmpworkdir.join('built_at')) == {'css': {'foo.css': '.foo{color:black}\n'}}
...
def test_sass_debug(tmpworkdir):
    mktree(
        ...
        build:
          sass:
            "css": "sass_dir"
        """,
            'sass_dir': {'foo.scss': '.foo { .bar {color: black;}}'},
        },
    )
    Grab().build()
    ...
    assert {
        ...
        'foo.css.map': RegexStr(r'{\n\t"version": 3,\n\t"file": "\.src/foo.css".*'),
        '.src': {'foo.scss': '.foo { .bar {color: black;}}'},
    } == tree
...
def test_rm_all(tmpworkdir):
    mktree(
        ...
        build:
          wipe: '.*'
        """,
            'built_at': {'foo/bar.js': 'x', 'boom.txt': 'x'},
        },
    )
    Grab().build()
    assert gettree(tmpworkdir.join('built_at')) == {}
...
def test_rm_some(tmpworkdir):
    ...
    setup_logging('DEBUG')
    Grab().build()
    assert {'foo': {'bar.js': 'x'}, 'remain.txt': 'y'} == gettree(tmpworkdir.join('built_at'))
    ...
    mktree(
        ...
        build:
          sass:
            "css": "sass_dir"
        """,
            'sass_dir': {'adir/test.scss': 'a { color: red};'},
        },
    )
    mock_import = mocker.patch('builtins.__import__')
    mock_import.side_effect = mocked_import
...
def test_sass_replace(tmpworkdir):
    mktree(
        ...
        """,
            'sass_dir': {'foo.scss': '.foo { .bar {color: black;}}', 'bar.scss': 'a {color: black;}'},
        },
    )
    Grab().build()
    assert {'foo.css': '.foo .bar{color:white}\n', 'bar.css': 'a{color:black}\n'} == gettree(
        tmpworkdir.join('built_at/css')
    )
...
def test_sass_clever_import(tmpworkdir):
    mktree(
        ...
        build:
          sass:
            css: sass_dir
        """,
            'downloaded2': {'x.css': '.x {width:100px}'},
            'sass_dir': {
    ...
    Grab().build()
    assert {'foo.css': 'a{color:black}.x{width:100px}\n'} == gettree(tmpworkdir.join('built_at/css'))
    ...
    mktree(
        ...
        build:
          sass:
            css: sass_dir
        """,
            'sass_dir': {'foo.scss': "@import 'SRC/bar';", '_bar.scss': 'a {color: black;}'},
        },
    )
    Grab().build()
    tree = gettree(tmpworkdir.join('built_at/css'))
    assert {
        '.src': {'foo.scss': "@import 'SRC/bar';", '_bar.scss': 'a {color: black;}'},
...
def test_sass_clever_import_node_modules(tmpworkdir):
    mktree(
        ...
        build:
          sass:
            css: sass_dir
        """,
            'sass_dir': {'foo.scss': "@import 'NM/foobar/bar';"},
            'node_modules': {'foobar/_bar.scss': 'a {color: black;}'},
        },
    )
    Grab().build()
    tree = gettree(tmpworkdir.join('built_at/css'))
    assert {
        'foo.css': 'a {\n  color: black; }\n\n/*# sourceMappingURL=foo.css.map */',
        'foo.css.map': RegexStr('{.*'),
        '.src': {'foo.scss': "@import 'NM/foobar/bar';"},
    } == tree


@pytest.mark.parametrize('value,result', [(0, '0B'), (1000, '1000B'), (2000, '2.0KB'), (1.2 * 1024 ** 2, '1.2MB')])
...
def test_raw_sass_prod(tmpdir):
    ...
    sass_gen()
    assert gettree(tmpdir.join('output')) == {'foo.css': '.foo{color:#G00D}\n'}
...
def test_raw_sass_prod_hash(tmpdir):
    ...
    sass_gen()
    assert gettree(tmpdir.join('output')) == {'foo.4f14ead.css': '.foo{color:#G00D}\n'}
...
def test_sass_custom_functions(tmpdir):
    mktree(tmpdir, {'sass': {'foo.scss': ".foo {color: waffle(100px, 'foobar.png')}"}, 'downloads': {}})
    args = None
...
def test_sass_custom_importer(tmpdir):
    mktree(tmpdir, {'sass': {'foo.scss': '@import "FOOBAR/hello"'}, 'downloads': {}})
    ...
    sass_gen()
    assert gettree(tmpdir.join('output')) == {'foo.css': 'div{color:red}\n'}
